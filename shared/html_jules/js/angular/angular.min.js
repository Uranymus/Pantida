...
(function (W, X, u) {
	...
	function Ca(b, a) {
		...
		if (c == typeof a && "object" == c)
			if (L(b)) {
				...
				if (va(b))
					return va(a) ? isNaN(b.getTime()) && isNaN(a.getTime()) || b.getTime() ===
									a.getTime() : !1;
				if (kb(b) && kb(a))
	...
	function gc(b, a) {
		...
		this.$get = ["$injector", "$interpolate", "$exceptionHandler", "$http", "$templateCache", "$parse", "$controller", "$rootScope", "$document", "$sce", "$animate", "$$sanitizeUri", function (a, b, l, n, q, p, s, J, w, t, y, K) {
				...
				function O(a, b, c, d, e, f) {
					...
					for (var k = [], p, l, m, q, n = 0; n < a.length; n++)
						p = new Ob, l = ca(a[n], [], p, 0 === n ? d : u, e), (f = l.length ? I(l, a[n], p, b, c, null, [], [], f) : null) && f.scope && ba(p.$$element, "ng-scope"), p = f && f.terminal || !(m = a[n].childNodes) || !m.length ?
					...
					for (var y = -Number.MAX_VALUE,
						...
						if (H.template)
							if (E = !0, fb("template", I, H, x), I = H, V = N(H.template) ? H.template(x, d) : H.template, V = W(V), H.replace) {
								...
								x.html(V);
						if (H.templateUrl)
							E = !0, fb("template", I, H, x), I = H, H.replace && (g = H), J = te(a.splice(Ha, a.length - Ha), x, d, f, F && S, k, q, {controllerDirectives: O, newIsolateScopeDirective: K, templateDirective: I, nonTlbTranscludeDirective: ea}), sa = a.length;
						else if (H.compile)
							try {
	...
	function Rd() {
		...
		this.$get = ["$parse", "$exceptionHandler", "$sce", function (c, d, e) {
				function f(f, m, l) {
					for (var n, q, p = 0, s = [], J = f.length, w = !1, t = []; p < J; )
						-1 != (n = f.indexOf(b, p)) && -1 != (q = f.indexOf(a,
	...
	function ma(b, a) {
		if (b) {
			if (b.constructor === b)
				throw la("isecfn", a);
			if (b.document &&
							b.location && b.alert && b.setInterval)
										...
										try {
											if (d = l[K])
												if ((f = d.get(O)) !== (h = d.last) && !(d.eq ? Ca(f, h) : "number" === typeof f && "number" === typeof h && isNaN(f) && isNaN(h)))
													B = !0, c = d, d.last = d.eq ? Ka(f, null) : f, d.fn(f, h === n ? f : h, O), 5 > u && (A = 4 - u, M[A] || (M[A] = []), P = N(d.exp) ? "fn: " + (d.exp.name || d.exp.toString()) : d.exp, P += "; newVal: " + oa(f) + "; oldVal: " +
																	oa(h), M[A].push(P));
												else if (d === c) {
													B = !1;
													break a
			...
			if (!l) {
				...
				if (R && "input" === (e || k).type && a[0].placeholder !== h)
					h = a[0].placeholder;
								...
								(f = $(f)), e = g && d.$$hasNativeValidators, d.$viewValue !== f || "" === f && e)
	...
	Ub.prototype = {constructor: Ub, lex: function (a) {
		...
		throwError: function (a, c, d) {
			...
			for (var d = "", e = a, f = !1; this.index < this.text.length; ) {
				...
				if (f)
					"u" === g ? (f = this.text.substring(this.index +
	...
	gb.prototype = {constructor: gb,
		...
		relational: function () {
			...
			var e = this;
			return function (f, g) {
				for (var h = [], k = c ? c(f, g) : f, m = 0; m < d.length; m++)
	...
	var sd = ["$animate", function (a) {
			...
			return{transclude: "element", priority: 1E3, terminal: !0, $$tlb: !0, link: function (e, f, g, h, k) {
					...
					e.$watchCollection(h, function (a) {
						...
						for (g = 0; g < h; g++)
							if (I = a ===
											v ? g : v[g], x = a[I], n = G(I, x, g), Ea(n, "`track by` id"), y.hasOwnProperty(n))
		...
		a.$watch(d.ngStyle, function (a, d) {
			...
			return{restrict: "E", require: ["select", "?ngModel"], controller: ["$element", "$scope", "$attrs", function (a, c, d) {
					...
					function n(e, f, g) {
						function h() {
							...
							for (F = c.length; Q < F; Q++) {
								...
								for (G = k.length; C < G; C++)
									d = k[C], (v = u[C + 1]) ? (E = v.element, v.label !== d.label && (E.text(v.label = d.label), E.prop("label", v.label)), v.id !== d.id && E.val(v.id = d.id), E[0].selected !== d.selected && (E.prop("selected",
						...
						var l = c(k[2] || k[1]),
										m = k[4] || k[6], n = k[5], r = c(k[3] || ""), x = c(k[2] ? k[1] : m), A = c(k[7]), w = k[8] ? c(k[8]) : null, B = [[{element: f, label: ""}]];
